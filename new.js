...
function sendSelection(){
    ...
    selectedRows.forEach(r=>{
        ...
        if(!seen.has(key)){
            seen.add(key);
            const timeStr = chartData[r.symbol].timeStr[chartData[r.symbol].y.indexOf(r.price)] || formatTimeOnly(new Date());
            uniqueRows.push({symbol: r.symbol, price: r.price, datetime: timeStr});
        }
    });
    fetch("/send_whatsapp",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({rows:uniqueRows})})
    .then(res=>res.json())
...
function sendChat(){
    ...
    fetch(`/current_price/${symbol}`).then(res=>res.json()).then(data=>{
